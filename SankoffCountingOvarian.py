...
class SankoffAlgorithm:
    ...
    def _find_optimal_at_root(self) -> Tuple[float, List[str]]:
        """
        Find the optimal cost and states at the root, 
        CONSTRAINED to only 'LOv' or 'ROv'.
        """
        root = self.tree.root
        
        # 1. Define your required constraints
        allowed_root_states = {'LOv', 'ROv', 'RUt'}
        
        # 2. Filter the calculated costs to only look at allowed states
        # We check if the state is in the allowed list AND exists in the cost matrix
        constrained_costs = {
            state: cost 
        ...
        # Safety check: ensure these states actually exist in your data
        if not constrained_costs:
            raise ValueError(
                f"Constraint Error: Neither 'LOv' nor 'ROv' found in root costs. "
                f"Available states were: {list(root.cost_matrix.keys())}"
            )

        # 3. Find minimum cost among ONLY the allowed states
        optimal_cost = min(constrained_costs.values())
        
        # 4. Find which of the allowed states achieve this minimum
        optimal_states = [
